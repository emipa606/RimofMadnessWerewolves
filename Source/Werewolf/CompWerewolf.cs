    ...
    public class CompWerewolf : CompAbilityUser
    {
        ...
        public void TransformBack(bool killed = false)
        {
            ...
            if (!Pawn.Dead && Pawn?.health?.hediffSet is HediffSet health)
            {
                ...
                for (var i = 0; i < (health?.GetInjuredParts().Count() ?? 0); i++)
                {
        ...
        private void ResolveBionics()
        {
            if (!Pawn.Dead)
            {
                foreach (BodyPartRecord rec in Pawn.health.hediffSet.GetNotMissingParts())
                {
                    ...
                    {
                        if (!WerewolfForms.Any(x => x.level >= 5) ||
                            ...
                            hediff_AddedPart?.def?.addedPartProps?.partEfficiency < 1f)
                        {
                            ...
                            {
                                Messages.Message(TranslatorFormattedStringExtensions.Translate("ROM_WerewolfDecyberize", Pawn.LabelShort, rec.def.label, hediff_AddedPart.Label), MessageTypeDefOf.NegativeEvent); //MessageTypeDefOf.NegativeEvent);
        ...
        {
            return addedPart is Vampire.Hediff_AddedPart_Fangs;
        }
        ...
        private void ResolveWerewolfStatMods(Pawn p, WerewolfForm currentWerewolfForm)
        {
        ...
        private void ResolveTransformEffects(Pawn p, WerewolfForm currentWerewolfForm, bool moonTransformation = false)
        {
            ...
            ResolveWerewolfStatMods(p, currentWerewolfForm);
            ResolveTransformedHediffs(p, currentWerewolfForm);
        ...
        private void ResolveWeaponStorage(Pawn_InventoryTracker inventory)
        {
                ...
                !equipment.NullOrEmpty())
            {
                ...
                {
                    if (Pawn.equipment.TryDropEquipment(c, out ThingWithComps d, Pawn.PositionHeld, false) && d != null)
                    {
                        StoredWeapons.Add(d);
        ...
        private void ResolveApparelStorage()
        {
            ...
            if (Pawn?.apparel?.WornApparel is List<Apparel> apparel && !apparel.NullOrEmpty())
            {
                ...
                foreach (Apparel c in temp)
                {
                    ...
                    {
                        if (Pawn.apparel.TryDrop(c, out Apparel d, Pawn.PositionHeld, false) && d != null)
                        {
        ...
        private void RestoreEquipment()
        {
            ...
            if (p?.inventory is Pawn_InventoryTracker invTracker)
            {
                ...
                if (!StoredWeapons.NullOrEmpty())
                {
                    ...
                    {
                        foreach (ThingWithComps c in storedItems)
                        {
        ...
        public override void PostExposeData()
        {
            ...
            Scribe_Collections.Look<ThingWithComps>(ref storedItems, "storedWeapons", LookMode.Reference);
